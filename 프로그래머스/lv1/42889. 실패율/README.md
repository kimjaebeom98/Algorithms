...
def solution(N, stages):
  ...
  for i in range(1, N+1):
    if people != 0:
      fallist[i] = stages.count(i) / people
      people -= stages.count(i)
    else:
      fallist[i] = 0

  
  return sorted(fallist.keys(), key=lambda i:fallist[i], reverse=True)
  
 ```
 
 - people은 해당 스테이지를 통과한 사람의 수를 나타내기 위해 사용. 예를 들어 2번 째 스테이지에 도달한 사람 수는
        people에서 첫 번째 스테이지를 통과한 사람의 수 만큼 뺀 수
 - dictionary의 key와 value 속성을 사용하여 index(stage)에 쉽게 접근 하도록 fallist dictionary를 선언
 - fallist의 keys가 곧 stages 니깐 정렬은 실패율의 내림차순으로 하되 출력물은 stage로 할 수 있도록 함
