...
풀이는 백준의 미로탐색과 거의 똑같다.
1. bfs는 시작 노드와 가까운 노드를 우선하여 탐색하므로 목표 노드에 도착하는 경로가 여러 개일 때, 최단 경로를 보장하는 성질을 이용하면 된다
2. 시작 노드(1, 1)에서 시작하여 상, 하, 좌, 우 네 방향을 탐색하는데 해당 방향이 invalid한 방향 (n행, m열)내의 좌표이고, 탐색을 하지 않았다면
+ 벽으로 막혀있지 않으면 그 방향으로 진행하면 됨
3. visited 배열을 갱신하는데 방문하지 않았으면 -1(초기화), 시작노드에서 부터 해당 좌표까지 오는 거리를 갱신한다
4. 마지막으로 실제로 마지막 지점에 도달했는 지 보기 위해 flag를 설정해서 실제 도달했으면 마지막 지점까지의 거리를 return해주고, 아니면 -1을 return
